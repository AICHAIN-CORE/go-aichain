...
import (
	...
	"github.com/AICHAIN-CORE/go-aichain/light"
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Stopping light AICHAIN protocol")
	...
	log.Info("Light AICHAIN protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Light AICHAIN peer connected", "name", p.Name())
	...
	if err := p.Handshake(td, hash, number, genesis.Hash(), pm.server); err != nil {
		p.Log().Debug("Light AICHAIN handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Light AICHAIN peer registration failed", "err", err)
		return err
	}
	defer func() {
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Light AICHAIN message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	p.Log().Trace("Light AICHAIN message arrived", "code", msg.Code, "bytes", msg.Size)
...
// NodeInfo represents a short summary of the AICHAIN sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network    uint64              `json:"network"`    // AICHAIN network ID (1=Frontier, 2=Morden, Ropsten=3, Rinkeby=4)

...
The dashboard is a data visualizer integrated into gait, intended to collect and visualize useful information of an AICHAIN node. It consists of two parts:
...
As the dashboard depends on certain NPM packages (which are not included in the `go-aichain` repo), these need to be installed first:
...
Normally the dashboard assets are bundled into Gait via `go-bindata` to avoid external dependencies. Rebuilding Gait after each UI modification however is not feasible from a developer perspective. Instead, we can run `webpack` in watch mode to automatically rebundle the UI, and ask `gait` to use external assets to not rely on compiled resources:

```
$ (cd dashboard/assets && ./node_modules/.bin/webpack --watch)
$ gait --dashboard --dashboard.assets=dashboard/assets --vmodule=dashboard=5
```

To bundle up the final UI into Gait, run `go generate`:

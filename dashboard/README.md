...
Normally the dashboard assets are bundled into Gait via `go-bindata` to avoid external dependencies. Rebuilding Gait after each UI modification however is not feasible from a developer perspective. Instead, we can run `yarn dev` to watch for file system changes and refresh the browser automatically.

```
$ gait --dashboard --vmodule=dashboard=5

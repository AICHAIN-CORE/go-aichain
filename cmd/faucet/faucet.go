...
import (
	...
	"github.com/AICHAIN-CORE/go-aichain/p2p/discv5"
...
var (
	...
	ethPortFlag = flag.Int("ethport", 30323, "Listener port for the devp2p connection")
	bootFlag    = flag.String("bootnodes", "", "Comma separated bootnode enode URLs to seed with")
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the AICHAIN protocol")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for AICHAIN and the faucet")
)
...
type request struct {
	...
	Account common.Address     `json:"account"` // AICHAIN address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an AICHAIN light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // AICHAIN protocol stack
	client *ethclient.Client   // Client connection to the AICHAIN chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the AICHAIN light client protocol
...
// close terminates the AICHAIN connection and tears down the faucet.
...
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	for {
		...
		// Retrieve the AICHAIN address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			err = errors.New("Something funky happened, please open an issue at https://github.com/AICHAIN-CORE/go-aichain/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
...
// the username, avatar URL and AICHAIN address to fund on success.
func authGitHub(url string) (string, string, common.Address, error) {
	...
	// Iterate over all the files and look for AICHAIN addresses
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No AICHAIN address found to fund")
	}
...
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the AICHAIN address and profile URL.
...
// returning the username, avatar URL and AICHAIN address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain AICHAIN address,

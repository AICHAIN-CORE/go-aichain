...
import (
	...
	bzzapi "github.com/AICHAIN-CORE/go-aichain/swarm/api"
	swarmmetrics "github.com/AICHAIN-CORE/go-aichain/swarm/metrics"
...
var (
	...
	SwarmSwapAPIFlag = cli.StringFlag{
		Name:   "swap-api",
		Usage:  "URL of the AICHAIN API provider to use to settle SWAP payments",
...
// This init function sets defaults so cmd/swarm can run alongside gait.
func init() {
...
var app = utils.NewApp("", "AICHAIN Swarm")

// This init function creates the cli.App.
func init() {
	app.Action = bzzd
	app.Version = sv.ArchiveVersion(gitCommit)
	app.Copyright = "Copyright 2013-2016 The go-aichain Authors"
			...
			Subcommands: []cli.Command{
				{
					...
					* use both --topic and --name to create named subtopics. 
						For example, --topic could be set to an AICHAIN contract address and --name could be set to "comments", meaning
...
Export a local chunk database as a tar archive (use - to send to stdout).
    ...
    swarm db export ~/.aichain/swarm/bzz-KEY/chunks chunks.tar

The export may be quite large, consider piping the output through the Unix
pv(1) tool to get a progress bar:

    swarm db export ~/.aichain/swarm/bzz-KEY/chunks - | pv > chunks.tar
`,
				},
				{
					Action:             dbImport,
    ...
    swarm db import ~/.aichain/swarm/bzz-KEY/chunks chunks.tar

The import may be quite large, consider piping the input through the Unix
pv(1) tool to get a progress bar:

    pv chunks.tar | swarm db import ~/.aichain/swarm/bzz-KEY/chunks -`,
				},
			},
		},
...
func bzzd(ctx *cli.Context) error {
	...
	//gait only supports --datadir via command line
	//in order to be consistent within swarm, if we pass --datadir via environment variable
	//or via config file, we get the same directory for gait and swarm
	...
	//register BZZ as node.Service in the aichain node
...
func registerBzzService(bzzconfig *bzzapi.Config, stack *node.Node) {
	...
	//register within the aichain node

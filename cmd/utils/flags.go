...
import (
	...
	"github.com/AICHAIN-CORE/go-aichain/common/fdlimit"
	...
	"github.com/AICHAIN-CORE/go-aichain/dashboard"
...
var (
	...
	NetworkIdFlag = cli.Uint64Flag{
		Name:  "networkid",
		Usage: "Network identifier (integer, 1=Frontier, 2=Morden (disused), 3=Ropsten, 4=Rinkeby, 1515=AiConsensus)",
	...
	AiConsensusFlag = cli.BoolFlag{
		Name:  "aiconsensus",
		Usage: "AiConsensus network: pre-configured AI Consensus network",
	}
	...
	// Ethash settings
	// we remove all settings of Ethhash for using lyra2DC!!!
...
func setBootstrapNodes(ctx *cli.Context, cfg *p2p.Config) {
	...
	case cfg.BootstrapNodes != nil:
		for _, node := range cfg.BootstrapNodes {
			fmt.Printf("bootnode ip=%s udpport=%d tcpport=%d\n", node.IP().String(), uint16(node.UDP()), uint16(node.TCP()))
		}
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses! (can search via `gait account list`)")
	log.Warn("-------------------------------------------------------------------")
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *eth.Config) {
	...
	case ctx.GlobalBool(AiConsensusFlag.Name):
		if !ctx.GlobalIsSet(NetworkIdFlag.Name) {
			cfg.NetworkId = 1515 //must set
		}
		cfg.Genesis = core.DefaultAiConsensusGenesisBlock()
	case ctx.GlobalBool(DeveloperFlag.Name):
		if !ctx.GlobalIsSet(NetworkIdFlag.Name) {
			cfg.NetworkId = 1337
		}
...
// RegisterEthService adds an AICHAIN client to the stack.
func RegisterEthService(stack *node.Node, cfg *eth.Config) {
	...
	if err != nil {
		Fatalf("Failed to register the AICHAIN service: %v", err)
	}
}
...
// RegisterEthStatsService configures the AICHAIN Stats daemon and adds it to
// th egiven node.
func RegisterEthStatsService(stack *node.Node, url string) {
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		// Retrieve both eth and les services
		var ethServ *eth.AICHAIN
		ctx.Service(&ethServ)
		...
		Fatalf("Failed to register the AICHAIN Stats service: %v", err)
	}
}
...
// new format.
//
// e.g. gait account new --keystore /tmp/mykeystore --lightkdf
//
// is equivalent after calling this method with:
//
// gait --keystore /tmp/mykeystore --lightkdf account new
//

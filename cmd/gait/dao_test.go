...
func testDAOForkBlockNewChain(t *testing.T, test int, genesis string, expectBlock *big.Int, expectVote bool) {
	...
	// Start a Gait instance with the requested flags set and immediately terminate
	if genesis != "" {
		...
		gait := runGeth(t, append(args, []string{"--exec", "2+2", "console"}...)...)
		gait.WaitExit()
	}
	// Retrieve the DAO config flag from the database
	path := filepath.Join(datadir, "gait", "chaindata")

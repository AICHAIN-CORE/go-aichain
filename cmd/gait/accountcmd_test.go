...
func TestAccountListEmpty(t *testing.T) {
	gait := runGeth(t, "account", "list")
	gait.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gait := runGeth(t, "account", "list", "--datadir", datadir)
	defer gait.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	gait := runGeth(t, "account", "new", "--lightkdf")
	defer gait.ExpectExit()
	gait.Expect(`
...
`)
	gait.ExpectRegexp(`Address: \{[0-9a-f]{40}\}\n`)
}
...
func TestWalletImport(t *testing.T) {
	gait := runGeth(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer gait.ExpectExit()
	gait.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(gait.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(gait.StderrText(), m) {
...
// https://github.com/AICHAIN-CORE/go-aichain/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gait.SetTemplateFunc("keypath", func(file string) string {

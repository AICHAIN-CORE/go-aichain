...
func TestConsoleWelcome(t *testing.T) {
	...
	// Start a gait console, make sure it's cleaned up and terminate the console
	gait := runGeth(t,
	...
	gait.Expect(`
Welcome to the Gait JavaScript console!

instance: Gait/v{{gethver}}/{{goos}}-{{goarch}}/{{gover}}
coinbase: {{.Etherbase}}
at block: 0 ({{niltime}})
 datadir: {{.Datadir}}
 modules: {{apis}}
...
func TestIPCAttachWelcome(t *testing.T) {
	...
	if runtime.GOOS == "windows" {
		ipc = `\\.\pipe\gait` + strconv.Itoa(trulyRandInt(100000, 999999))
	} else {
		ws := tmpdir(t)
		defer os.RemoveAll(ws)
		ipc = filepath.Join(ws, "gait.ipc")
	}
	...
	testAttachWelcome(t, gait, "ipc:"+ipc, ipcAPIs)
...
func TestHTTPAttachWelcome(t *testing.T) {
	...
	time.Sleep(2 * time.Second) // Simple way to wait for the RPC endpoint to open
	testAttachWelcome(t, gait, "http://localhost:"+port, httpAPIs)
...
func TestWSAttachWelcome(t *testing.T) {
	...
	time.Sleep(2 * time.Second) // Simple way to wait for the RPC endpoint to open
	testAttachWelcome(t, gait, "ws://localhost:"+port, httpAPIs)

	gait.Interrupt()
	gait.ExpectExit()
}

func testAttachWelcome(t *testing.T, gait *testgeth, endpoint, apis string) {
	// Attach to a running gait note and terminate immediately
	...
	attach.SetTemplateFunc("etherbase", func() string { return gait.Etherbase })
	attach.SetTemplateFunc("niltime", func() string { return time.Unix(0, 0).Format(time.RFC1123) })
	attach.SetTemplateFunc("ipc", func() bool { return strings.HasPrefix(endpoint, "ipc") })
	attach.SetTemplateFunc("datadir", func() string { return gait.Datadir })

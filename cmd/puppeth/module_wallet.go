...
RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'gait --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'exec gait --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --ethstats \'{{.Ethstats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*" --rpcvhosts "*"' >> wallet.sh
...
services:
  wallet:
    ...
    ports:
      ...
      - "{{.RPCPort}}:9523"{{if not .VHost}}
      - "{{.WebPort}}:80"{{end}}
    volumes:
      - {{.Datadir}}:/root/.aichain
    environment:
...
func checkWallet(client *sshClient, network string) (*walletInfos, error) {
	...
	rpcPort := infos.portmap["9523/tcp"]

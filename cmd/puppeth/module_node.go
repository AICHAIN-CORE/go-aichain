...
// nodeDockerfile is the Dockerfile required to run an AICHAIN node.
var nodeDockerfile = `
FROM aichain/client-go:latest
...
{{end}}
RUN \
  echo 'gait --cache 512 init /genesis.json' > gait.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.aichain/keystore/ && cp /signer.json /root/.aichain/keystore/' >> gait.sh && \{{end}}
	echo $'exec gait --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --nat extip:{{.IP}} --maxpeers {{.Peers}} {{.LightFlag}} --ethstats \'{{.Ethstats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Etherbase}}--miner.etherbase {{.Etherbase}} --mine --miner.threads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --miner.gastarget {{.GasTarget}} --miner.gaslimit {{.GasLimit}} --miner.gasprice {{.GasPrice}}' >> gait.sh

ENTRYPOINT ["/bin/sh", "gait.sh"]
`

// nodeComposefile is the docker-compose.yml file required to deploy and maintain
// an AICHAIN node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.aichain{{if .Ethashdir}}
...
// deployNode deploys a new AICHAIN node container to a remote machine via SSH,
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 gait --exec admin.nodeInfo.enode --cache=16 attach", network, kind)); err != nil {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.aichain"],

...
var (
	// Files that end up in the gait*.zip archive.
	gethArchiveFiles = []string{
		"COPYING",
		executablePath("gait"),
	}

	// Files that end up in the gait-alltools*.zip archive.
...
func doDebianSource(cmdline []string) {
	var (
		...
		upload  = flag.String("upload", "", `Where to upload the source package (usually "aichain/aichain")`)
		sshUser = flag.String("sftp-user", "", `Username for SFTP upload (usually "gait-ci")`)
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = ioutil.TempDir("", "gait-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, file := range allToolsArchiveFiles {
		...
		if filepath.Base(file) == "gait.exe" {
			gethTool = file
		} else {
	...
	// first section contains the gait binary, second section holds the dev tools.
	templateData := map[string]interface{}{
		"License":  "COPYING",
		"Gait":     gethTool,
		"DevTools": devTools,
	}
	build.Render("build/nsis.gait.nsi", filepath.Join(*workdir, "gait.nsi"), 0644, nil)
	...
	build.MustRunCommand("makensis.exe",
		...
		"/DARCH="+*arch,
		filepath.Join(*workdir, "gait.nsi"),
	)
...
func doAndroidArchive(cmdline []string) {
	...
	if *local {
		...
		os.Rename("gait.aar", filepath.Join(GOBIN, "gait.aar"))
		return
	}
...
func doXCodeFramework(cmdline []string) {
	...
	if *deploy != "" {
		...
		build.Render("build/pod.podspec", "Gait.podspec", 0755, meta)
		build.MustRunCommand("pod", *deploy, "push", "Gait.podspec", "--allow-warnings", "--verbose")
	}
}

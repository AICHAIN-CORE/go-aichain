...
// Service implements an AICHAIN netstats reporting daemon that pushes local
// chain statistics up to a monitoring server.
type Service struct {
	server *p2p.Server        // Peer-to-peer server to retrieve networking infos
	eth    *eth.AICHAIN      // Full AICHAIN service if monitoring a full node
	les    *les.LightEthereum // Light AICHAIN service if monitoring a light node
...
func New(url string, ethServ *eth.AICHAIN, lesServ *les.LightEthereum) (*Service, error) {

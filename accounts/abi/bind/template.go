...
import "github.com/AICHAIN-CORE/go-aichain/accounts/abi"
...
import (
	...
	aichain "github.com/AICHAIN-CORE/go-aichain"
	"github.com/AICHAIN-CORE/go-aichain/accounts/abi"
...
{{range $contract := .Contracts}}
	...
	{{if .InputBin}}
		...
		// Deploy{{.Type}} deploys a new AICHAIN contract, binding an instance of {{.Type}} to it.
	...
	// {{.Type}} is an auto generated Go binding around an AICHAIN contract.
	type {{.Type}} struct {
	...
	// {{.Type}}Caller is an auto generated read-only Go binding around an AICHAIN contract.
	type {{.Type}}Caller struct {
	  contract *bind.BoundContract // Generic contract wrapper for the low level calls
	}

	// {{.Type}}Transactor is an auto generated write-only Go binding around an AICHAIN contract.
	type {{.Type}}Transactor struct {
	  contract *bind.BoundContract // Generic contract wrapper for the low level calls
	}

	// {{.Type}}Filterer is an auto generated log filtering Go binding around an AICHAIN contract events.
	type {{.Type}}Filterer struct {
	  contract *bind.BoundContract // Generic contract wrapper for the low level calls
	}

	// {{.Type}}Session is an auto generated Go binding around an AICHAIN contract,
	...
	// {{.Type}}CallerSession is an auto generated read-only Go binding around an AICHAIN contract,
	...
	// {{.Type}}TransactorSession is an auto generated write-only Go binding around an AICHAIN contract,
	...
	// {{.Type}}Raw is an auto generated low-level Go binding around an AICHAIN contract.
	type {{.Type}}Raw struct {
	  Contract *{{.Type}} // Generic contract binding to access the raw methods on
	}

	// {{.Type}}CallerRaw is an auto generated low-level read-only Go binding around an AICHAIN contract.
	type {{.Type}}CallerRaw struct {
		Contract *{{.Type}}Caller // Generic read-only contract binding to access the raw methods on
	}

	// {{.Type}}TransactorRaw is an auto generated low-level write-only Go binding around an AICHAIN contract.
	...
	{{range .Events}}
		...
		type {{$contract.Type}}{{.Normalized.Name}}Iterator struct {
			...
			sub  aichain.Subscription // Subscription for errors, completion and termination
...
import org.aichain.gait.*;
import org.aichain.gait.internal.*;
...
{{range $contract := .Contracts}}
	public class {{.Type}} {
		...
		{{if .InputBin}}
			...
			// deploy deploys a new AICHAIN contract, binding an instance of {{.Type}} to it.
			public static {{.Type}} deploy(TransactOpts auth, EthereumClient client{{range .Constructor.Inputs}}, {{bindtype .Type}} {{.Name}}{{end}}) throws Exception {
				Interfaces args = Gait.newInterfaces({{(len .Constructor.Inputs)}});
				{{range $index, $element := .Constructor.Inputs}}
				  args.set({{$index}}, Gait.newInterface()); args.get({{$index}}).set{{namedtype (bindtype .Type) .Type}}({{.Name}});
				{{end}}
				return new {{.Type}}(Gait.deployContract(auth, ABI, BYTECODE, client, args));
			}
		...
		// AICHAIN address where this contract is located at.
		public final Address Address;

		// AICHAIN transaction in which this contract was deployed (if known!).
		...
		public {{.Type}}(Address address, EthereumClient client) throws Exception {
			this(Gait.bindContract(address, ABI, client));
		}
		...
		{{range .Calls}}
			...
			public {{if gt (len .Normalized.Outputs) 1}}{{capitalise .Normalized.Name}}Results{{else}}{{range .Normalized.Outputs}}{{bindtype .Type}}{{end}}{{end}} {{.Normalized.Name}}(CallOpts opts{{range .Normalized.Inputs}}, {{bindtype .Type}} {{.Name}}{{end}}) throws Exception {
				Interfaces args = Gait.newInterfaces({{(len .Normalized.Inputs)}});
				{{range $index, $item := .Normalized.Inputs}}args.set({{$index}}, Gait.newInterface()); args.get({{$index}}).set{{namedtype (bindtype .Type) .Type}}({{.Name}});
				{{end}}

				Interfaces results = Gait.newInterfaces({{(len .Normalized.Outputs)}});
				{{range $index, $item := .Normalized.Outputs}}Interface result{{$index}} = Gait.newInterface(); result{{$index}}.setDefault{{namedtype (bindtype .Type) .Type}}(); results.set({{$index}}, result{{$index}});
				{{end}}

				if (opts == null) {
					opts = Gait.newCallOpts();
				}

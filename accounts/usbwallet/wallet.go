...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the AICHAIN
...
type wallet struct {
	...
	deriveChain    aichain.ChainStateReader // Blockchain state reader to discover used account with
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived AICHAIN account
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain aichain.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
...
// data is not supported for Ledger wallets, so this method will always return
// an error.
func (w *wallet) SignHashWithPassphraseCached(account accounts.Account, passphrase string, hash []byte) ([]byte, error) {
	return w.SignHash(account, hash)
}

...
[https://swarm.aichain.org](https://swarm.aichain.org)
...
To run Swarm you need an AICHAIN account. You can create a new account by running the following command:
...
Once you have specified the password, the output will be the AICHAIN address representing that account. For example:
...
### AICHAIN Name Service resolution

The AICHAIN Name Service is the AICHAIN equivalent of DNS in the classic web. In order to use ENS to resolve names to Swarm content hashes (e.g. `bzz://theswarm.eth`), `swarm` has to connect to a `gait` instance, which is synced with the AICHAIN mainnet. This is done using the `--ens-api` flag.

    swarm --bzzaccount <your-account-here> \
          --ens-api '$HOME/.aichain/gait.ipc'
...
You must have your working copy under `$GOPATH/src/github.com/AICHAIN-CORE/go-aichain`.

Most likely you will be working from your fork of `go-aichain`, let's say from `github.com/nirname/go-aichain`. Clone or move your fork into the right place:

```
git clone git@github.com:nirname/go-aichain.git $GOPATH/src/github.com/AICHAIN-CORE/go-aichain
```
...
```
stateth --rm --grafana-dashboards-folder $GOPATH/src/github.com/AICHAIN-CORE/go-aichain/swarm/grafana_dashboards --influxdb-database metrics
```
...
Swarm offers a local HTTP proxy API that Dapps can use to interact with Swarm. The AICHAIN Foundation is hosting a public gateway, which allows free access so that people can try Swarm without running their own node.
 ...
 * [Code review guidelines](https://github.com/AICHAIN-CORE/go-aichain/wiki/Code-Review-Guidelines).

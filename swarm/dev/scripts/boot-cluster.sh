...
#
# The cluster contains a bootnode, a gait node and multiple swarm nodes, with
...
# start_bootnode starts a bootnode which is used to bootstrap the gait and
# swarm nodes
start_bootnode() {
...
# start_geth_node starts a gait node with --datadir pointing at <base-dir>/gait
# and a single, unlocked account with password "gait"
start_geth_node() {
  local dir="${base_dir}/gait"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get gait account address"
  fi
  ...
  start_node "gait" "${GETH_IP}" "$(which gait)" ${args[@]}
}
...
create_account() {
  ...
  gait --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}

...
`pss` is under active development, and the first implementation is yet to be merged to the AICHAIN main branch. Expect things to change.
...
The code tutorial [p2p programming in go-aichain](https://github.com/nolash/go-aichain-p2p-demo) by [@nolash](https://github.com/nolash) provides step-by-step code examples for usage of `pss` API with `go-aichain` nodes.
...
For `golang` clients, please use the `rpc.Client` provided by the `go-aichain` repository. The return values may have special types in `golang`. Please refer to `godoc` for details.

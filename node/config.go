...
import (
	...
	"github.com/AICHAIN-CORE/go-aichain/accounts/usbwallet"
...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of gait is "gait". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Gait", keep that.
	if name == "gait" || name == "gait-testnet" {
		name = "Gait"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "gait" instances.
...
func (c *Config) ResolvePath(path string) string {
	...
	// by gait 1.4 are used if they exist.
	if warn, isOld := isOldGethResource[path]; isOld {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldGethResourceWarning, "Using deprecated resource file %s, please move this file to the 'gait' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "go-aichain-keystore")
		ephemeral = keydir
	}

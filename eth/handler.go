...
import (
	...
	"github.com/AICHAIN-CORE/go-aichain/eth/fetcher"
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing AICHAIN peer", "peer", id)

	// Unregister the peer from the downloader and AICHAIN peer set
...
func (pm *ProtocolManager) Stop() {
	log.Info("Stopping AICHAIN protocol")
	...
	log.Info("AICHAIN protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("AICHAIN peer connected", "name", p.Name())

	// Execute the AICHAIN handshake
	var (
	...
	if err := p.Handshake(pm.networkID, td, hash, genesis.Hash()); err != nil {
		p.Log().Debug("AICHAIN handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("AICHAIN peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("AICHAIN message handling failed", "err", err)
			return err
		}
	}
}

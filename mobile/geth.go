...
// entire API provided by go-aichain to reduce the maintenance surface and dev
// complexity.
type NodeConfig struct {
	...
	// EthereumEnabled specifies whether the node should run the AICHAIN protocol.
	EthereumEnabled bool

	// EthereumNetworkID is the network identifier used by the AICHAIN protocol to
...
// Node represents a Gait AICHAIN node instance.
type Node struct {
	node *node.Node
}

// NewNode creates and configures a new Gait node.
func NewNode(datadir string, config *NodeConfig) (stack *Node, _ error) {
	...
	// Register the AICHAIN protocol if requested
	if config.EthereumEnabled {
		...
		if err := rawStack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			...
			return nil, fmt.Errorf("aichain init: %v", err)
		}
...
// GetEthereumClient retrieves a client to access the AICHAIN subsystem.
